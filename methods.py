...
def import_my_files(case, base, data, company_type):
    """
    Read files.ini
    ...
    config.read(base_path.joinpath(r'input_files.ini'))
    ...
    for key in config.keys():
        ...
        my_files[key] = {'PATH': case.joinpath(prefix + config.get(key, 'PATH')),
                         'FIN_PATH': case.joinpath(prefix + config.get(key, 'FIN_PATH')),
                         'METHOD_PATH': case.joinpath(prefix + config.get(key, 'METHOD_PATH')),
                         'SOURCE_PATH': case.joinpath(prefix + config.get(key, 'SOURCE_PATH')),
                         'SCREENING_PATH': case.joinpath(prefix + config.get(key, 'SCREENING_PATH')),
                         'EXPOSURE_PATH': case.joinpath(prefix + config.get(key, 'EXPOSURE_PATH')),
                         'RND_PATH': case.joinpath(prefix + config.get(key, 'RND_PATH'))
                         }

    return my_files


def create_country_map(case_root, map_source_path):
    """
    ...
    print(map_source_path)

    # Save it as csv
    os.mkdir(case_root.joinpath(r'Mapping'))

    country_map.to_csv(map_source_path,
                       index=False,
...
def select_main(config, files, country_map):
    """
    ...
    for region in config['REGIONS']:
        i += 1
        print(region + ' (File #' + str(i) + '/' + str(len(config['REGIONS'])) + ')')

        # Read input list of companies by world region
        df = pd.read_excel(config['CASE_ROOT'].joinpath(r'input\listed companies - ' + region + '.xlsx'),
                           sheet_name='Results',
        ...
        df['y_lastav'] = config['YEAR_LASTAV']

        df['rnd_mean'] = df[['rnd_y' + str(YY) for YY in range(10, 19)]].mean(axis=1, skipna=True)

        df['rnd_y_lastav'] = df['rnd_y' + str(abs(config['YEAR_LASTAV']) % 100)]
...
def load_main_comp_fin(config, files, select_comp):
    """
    ...
    for number in list(range(1, config['MAIN_COMPS_FIN_FILE_N'] + 1)):
        print('File #' + str(number) + '/' + str(config['MAIN_COMPS_FIN_FILE_N']))
        df = pd.read_excel(
            config['CASE_ROOT'].joinpath(r'input\listed companies - financials #' + str(number) + '.xlsx'),
            sheet_name='Results',
    ...
    report = report.append(
        pd.DataFrame(
            {'Selected_bvd9': main_comp_fin['bvd9'].nunique(),
             'Selected_rnd_y' + str(config['YEAR_LASTAV'])[-2:]: main_comp_fin[
                 'rnd_y' + str(config['YEAR_LASTAV'])[-2:]].sum()
             }, index=['Initial']
        ))
...
def select_subs(config, files):
    """
    Consolidate a unique list of subsidiaries
    :param case_root: path of the working folder for the use case
    :param config['SUBS_ID_FILE_N']: Number of input files to consolidate
    :return: analytical report
    """
    # Initialize DF
    subs = pd.DataFrame()
    ...
    for number in list(range(1, config['SUBS_ID_FILE_N'] + 1)):
        print('File #' + str(number) + '/' + str(config['SUBS_ID_FILE_N']))
        df = pd.read_excel(
            config['CASE_ROOT'].joinpath(r'input\listed companies subsidiaries #' + str(number) + '.xlsx'),
            sheet_name='Results',
...
def load_subs_fin(config, files, select_subs, country_map):
    """
    ...
    for number in list(range(1, config['SUBS_FIN_FILE_N'] + 1)):
        print('File #' + str(number) + '/' + str(config['SUBS_FIN_FILE_N']))
        df = pd.read_excel(
            config['CASE_ROOT'].joinpath(r'input\listed companies subsidiaries - financials #' + str(number) + '.xlsx'),
            sheet_name='Results',
...
def filter_comps_and_subs(config, files, select_subs, subs_fin):
    """
...
def screen_subs(config, files, keywords, subs_fin):
...
def compute_sub_exposure(config, files, select_subs, screen_subs):
...
def compute_main_comp_rnd(config, files, main_comp_exposure, main_comp_fin):
...
def compute_sub_rnd(config, files, sub_exposure, main_comp_rnd):
    sub_rnd_conso = pd.DataFrame()

    for method in config['METHODS']:
        # Calculating subsidiary level rnd
        sub_rnd = pd.merge(
            sub_exposure, main_comp_rnd[['bvd9', 'main_comp_rnd', 'year', 'main_comp_rnd_final']],
            left_on='bvd9', right_on='bvd9',
            how='left'
        )
