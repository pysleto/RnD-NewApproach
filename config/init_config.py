...
with open(local.project_path.joinpath('config', 'registry.py'), 'w') as file:
    ...
    parent_bvd9_short_path = os.fspath(local.case_path.joinpath('parents - select -' + config.get('DEFAULT', 'out_bvd9') + '.csv'))
    ...
    guo_rnd_path = os.fspath(local.case_path.joinpath('guos - ' + config.get('DEFAULT', 'out_rnd') + '.csv'))
    ...
    file.write('parent_bvd9_collect_path' + ' = ' + 'Path(r' + repr(str(parent_bvd9_full_path)) + ')' + '\n')
    file.write('parent_bvd9_select_path' + ' = ' + 'Path(r' + repr(str(parent_bvd9_short_path)) + ')' + '\n')
    ...
    sub_collect_files_n = ast.literal_eval(config.get(local.use_case, 'sub_collect_files_n'))
    ...
    file.write('sub_collect_files_n' + ' = ' + str(sub_collect_files_n) + '\n')
