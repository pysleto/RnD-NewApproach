...
print('\n#0 - Identify and collect parent companies \n')
...
print('\nparent_bvd9:' + str(
    pd.Series(parent_ids_collection['bvd9'].unique()).count()))
print('guo_bvd9:' + str(
    pd.Series(parent_ids_collection['guo_bvd9'].unique()).count()))
# </editor-fold>

# <editor-fold desc="#1 - Select parent companies">
print('\n#1 - Select and consolidate parent companies \n')
...
if not (reg.parent_id_path.exists() & reg.guo_id_path.exists()):
    ...
    # TODO : Load specific consolidation guo ids
    ...
    parent_ids['parent_unique_id'] = parent_ids['country_2DID_iso'] + parent_ids['bvd9'] + parent_ids['parent_conso']

    parent_ids.to_csv(reg.parent_id_path,
                      columns=['parent_unique_id'] + col.comp_ids['consolidation']['parent'] + ['world_player'],
                      float_format='%.10f',
                      index=False,
                      na_rep='#N/A'
                      )
    ...
    # TODO: Re activate when specific guo id consolidation is implemented
    # guo_ids = by_mtd.select_by_account(guo_ids, 'guo', reg.account_conso_type)

    guo_ids.to_csv(reg.guo_id_path,
                   columns=col.comp_ids['consolidation']['guo'],
                   float_format='%.10f',
                   index=False,
                   na_rep='#N/A'
                   )
...
if not (reg.sub_bvd9_collect_path.exists() & reg.sub_bvd9_all_path.exists()):
    # (report['load_subsidiary_identification'], sub_ids) = rd_mtd.load_sub_ids()
    sub_ids_collect = rd_mtd.load_sub_ids('collection')

    sub_ids_collect['parent_unique_id'] = sub_ids_collect['country_2DID_iso'] + sub_ids_collect['bvd9'] + \
                                          sub_ids_collect['parent_conso']

    sub_ids_collect = sub_ids_collect[sub_ids_collect['parent_unique_id'].isin(parent_ids['parent_unique_id'])]

    print('Save output file - sub_ids_collect table ... ')

    sub_ids_collect.to_csv(reg.sub_bvd9_collect_path,
                           columns=['parent_unique_id'] + col.comp_ids['collection']['sub'],
                           index=False,
                           na_rep='#N/A'
                           )
    ...
    sub_bvd9_ids.to_csv(reg.sub_bvd9_all_path,
                        header=False,
                        index=False,
                        na_rep='#N/A'
                        )

print('Read file - sub_ids_collect table ... ')

sub_ids_collect = pd.read_csv(
...
print('all_sub_bvd9:' + str(pd.Series(sub_ids_collect.sub_bvd9.unique()).count()))
# </editor-fold>
...
print('\n#2b - Consolidate subsidiary identification \n')

if not reg.sub_id_path.exists():
    # (report['load_subsidiary_identification'], sub_ids) = rd_mtd.load_sub_ids()
    sub_ids = rd_mtd.load_sub_ids('consolidation')

    sub_ids = by_mtd.select_by_account(sub_ids, 'sub', reg.account_conso_all)

    # Merge with guo_id
    sub_ids = pd.merge(
        sub_ids_collect[['parent_unique_id', 'country_2DID_iso', 'bvd9', 'parent_conso', 'company_name', 'sub_bvd9', 'sub_lvl']],
        sub_ids,
    ...
    sub_ids['sub_unique_id'] = sub_ids['sub_country_2DID_iso'] + sub_ids['sub_bvd9'] + sub_ids['sub_conso']
    ...
    sub_ids.to_csv(reg.sub_id_path,
                   columns=col.comp_ids['consolidation']['sub_ext'],
                   float_format='%.10f',
                   index=False,
                   na_rep='#N/A'
                   )
...
if not reg.parent_fin_path.exists():
    parent_fins = rd_mtd.load_comp_fins('parent')
    ...
    parent_fins['parent_unique_id'] = parent_fins['country_2DID_iso'] + parent_fins['bvd9'] + parent_fins['parent_conso']

    parent_fins = parent_fins[parent_fins['parent_unique_id'].isin(parent_ids['parent_unique_id'])]

    # Save it as csv
    parent_fins.to_csv(reg.parent_fin_path,
                       columns=['parent_unique_id'] + col.comp_fins['parent'],
                       float_format='%.10f',
                       index=False,
                       na_rep='#N/A'
                       )
...
print('Save output for excel - parent_fins table ... ')

# Save it as csv
parent_fins.to_csv(reg.case_path.joinpath('parents - financials_for_xls.csv'),
                   columns=['parent_unique_id'] + ['bvd9', 'parent_conso', 'country_2DID_iso'] + \
...
if not reg.sub_fin_path.exists():
    sub_fins = rd_mtd.load_comp_fins('sub')

    sub_fins.sort_values(by=['oprev_sum', 'rnd_sum'], ascending=False, na_position='last', inplace=True)

    sub_fins = by_mtd.select_by_account(sub_fins, 'sub', reg.account_conso_all)

    sub_fins['sub_unique_id'] = sub_fins['sub_country_2DID_iso'] + sub_fins['sub_bvd9'] + sub_fins['sub_conso']

    sub_fins = sub_fins[sub_fins['sub_unique_id'].isin(sub_ids['sub_unique_id'])]
    ...
    sub_fins.to_csv(reg.sub_fin_path,
                    columns=['sub_unique_id'] + col.comp_fins['sub'] +
...
sub_fins.to_csv(reg.case_path.joinpath('subsidiaries - financials_for_xls.csv'),
                columns=['sub_unique_id'] + ['sub_bvd9', 'sub_conso', 'rnd_sum', 'oprev_sum'] + reg.oprev_ys_for_exp[::-1] + [
                    'sub_turnover_sum', 'sub_turnover_sum_masked', 'keyword_mask'],
                float_format='%.10f',
                index=False,
                na_rep='#N/A'
...
# sub_rnd = pd.merge(
#     sub_rnd,
#     parent_ids[['bvd9', 'company_name', 'parent_conso', 'is_quoted', 'country_2DID_iso', 'world_player',
#                 'guo_bvd9']],
...
#                   'world_player': 'parent_world_player', 'is_quoted': 'is_parent_quoted'})

# sub_rnd = pd.merge(
#     sub_rnd,
#     guo_ids[
#         ['guo_bvd9', 'guo_name', 'guo_conso', 'is_quoted', 'guo_country_2DID_iso', 'guo_world_player']],
#     left_on='guo_bvd9', right_on='guo_bvd9',
#     how='left',
#     suffixes=(False, False)
