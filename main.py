...
files = mtd.import_my_files(config['CASE_ROOT'], base, data, company_type)
...
if not files['MAP']['PATH'].exists():
    mtd.create_country_map(config['CASE_ROOT'], files['MAP']['SOURCE_PATH'])

print('Read country_table.csv - Country mapping table ...')
country_map = pd.read_csv(files['MAP']['PATH'])
# </editor-fold>
...
if not files['MAIN_COMPS']['PATH'].exists():
    report = mtd.select_main(config, files, country_map)
...
if not files['MAIN_COMPS']['FIN_PATH'].exists():
    report = mtd.load_main_comp_fin(config, files, select_comp)
...
if not files['SUBS']['PATH'].exists():
    report = mtd.select_subs(config, files)
...
if not files['SUBS']['FIN_PATH'].exists():
    report = mtd.load_subs_fin(config, files, select_subs, country_map)
...
if not files['SUBS']['METHOD_PATH'].exists():
    report = mtd.filter_comps_and_subs(config, files, select_subs, subs_fin)
...
select_subs = pd.read_csv(
    ...
    usecols=['company_name', 'bvd9', 'sub_company_name', 'sub_bvd9'] + config['METHODS']
)
# </editor-fold>
...
if not files['SUBS']['SCREENING_PATH'].exists():
    report = mtd.screen_subs(config, files, keywords, subs_fin)
...
if not files['SUBS']['EXPOSURE_PATH'].exists():
    mtd.compute_sub_exposure(config, files, select_subs, screen_subs)
...
if not files['MAIN_COMPS']['RND_PATH'].exists():
    mtd.compute_main_comp_rnd(config, files, main_comp_exposure, main_comp_fin)
...
if not files['SUBS']['RND_PATH'].exists():
    mtd.compute_sub_rnd(config, files, sub_exposure, main_comp_rnd)

print('# Read ' + str(company_type) + ' subisdiaries - rnd estimates.csv ...')
