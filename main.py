...
# Load config files
case = mtd.import_my_case(use_case, base, data)
files = mtd.import_my_files(case['CASE_ROOT'], base, data, company_type)

# Load keywords for activity screening
with open(case['BASE'].joinpath(r'keywords.json'), 'r') as file:
...
if files['REPORT']['PATH'].exists():
    ...
    #    pprint(case, stream=r)
    r.write('\n')

# Import mapping tales
if not files['MAP']['PATH'].exists():
    mtd.create_country_map(case, files)
...
if not files['MAIN_COMPS']['PATH'].exists():
    report = mtd.select_main(case, files, country_map)

    r.write('Step #1 - Selection of main companies\n\n')
    r.write(str(case['YEAR_LASTAV']) + ' ¦ ' + company_type + ' ¦ ' + 'RnD in EUR million\n\n')
...
if not files['MAIN_COMPS']['FIN_PATH'].exists():
    report = mtd.load_main_comp_fin(case, files, select_comp)
...
if not files['SUBS']['PATH'].exists():
    report = mtd.select_subs(case, files)
...
if not files['SUBS']['FIN_PATH'].exists():
    report = mtd.load_subs_fin(case, files, select_subs, country_map)
...
if not files['SUBS']['METHOD_PATH'].exists():
    report = mtd.filter_comps_and_subs(case, files, select_subs, subs_fin)
...
select_subs = pd.read_csv(
    ...
    usecols=['company_name', 'bvd9', 'sub_company_name', 'sub_bvd9'] + case['METHODS']
)
# </editor-fold>
...
if not files['SUBS']['SCREENING_PATH'].exists():
    report = mtd.screen_subs(case, files, keywords, subs_fin)
...
if not files['SUBS']['EXPOSURE_PATH'].exists():
    mtd.compute_sub_exposure(case, files, select_subs, screen_subs)
...
if not files['MAIN_COMPS']['RND_PATH'].exists():
    mtd.compute_main_comp_rnd(case, files, main_comp_exposure, main_comp_fin)
...
if not files['SUBS']['RND_PATH'].exists():
    mtd.compute_sub_rnd(case, files, sub_exposure, main_comp_rnd)
